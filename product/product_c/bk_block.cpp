...
private:
    disp_t *frames[3]; // num of frames must be 3.
    std::atomic<capture_state> states[3];
protected:
    int acquire_writable_idx() {
        for(int i = 0;i < NUM_OF_CAP_FRAMES;i++) {
            if(states[i] == INVALID || states[i] == READY) {
                if(std::atomic::compare_exchange_weak(&states[i], INVALID, WRITING) || std::atomic::compare_exchange_weak(&states[i], READY, WRITING)) {
                    return i;
                }
            }
        }
        xil_printf("[!!!!!!]: no writable frame\n");
        return -1;
    }
public:
    capture_manager(disp_t *frame1[NUM_OF_CAP_FRAMES]) {
        for(int i = 0;i < NUM_OF_CAP_FRAMES;i++) {
            frames[i] = frame1[i];
            states[i] = INVALID;
        }
